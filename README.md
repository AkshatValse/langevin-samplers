...
Langevin sampler, set up so that every measured quantity has a closed-form answer to be
checked against.
...
`X_{k+1} = X_k + h ∇log π(X_k) + √(2h) ξ_k`, is ergodic, but its invariant law `π_h` is
not `π`, and the gap is the discretization bias. Metropolizing the same move (MALA)
...
1. Gaussian bias (`scripts/01`). ULA and MALA over a decade and a half of step sizes on
   an anisotropic Gaussian (`s = 0.5, 1, 2`; 4:1 conditioning, stability limit
...
2. Metastability (`scripts/02`). A separated two-component mixture, sweeping the
   *barrier* `Δ(μ) = μ²/2s² − log 2` rather than the step size, with every chain started
   in the left well. Kramers' law predicts a crossing rate scaling like `exp(−Δ)`, and
   the measured rates recover that slope. Past `μ ≈ 4` both samplers stop crossing
   entirely and report the well they were started in. The Metropolis correction buys
   nothing here, since MALA is unbiased *in stationarity* and a chain that has not
   crossed is nowhere near stationarity. Bias is polynomial in `h` and can be corrected.
   Mixing is exponential in `Δ` and cannot, so fixing the first does nothing for the
   second.

## Results

The sampler reproduces its own exact stationary law. Across 10 step sizes and 3
coordinates, the measured ULA variance agrees with `v(h) = s²/(1 − h/2s²)` in 30 of 30
cases to within 3 Monte Carlo standard errors, at a Monte Carlo error of 0.14 to 0.29%.
...
Mixing is exponential in the barrier, and the correction does not touch it. Fitting
`log(crossing rate)` against `Δ = μ²/2s² − log 2` gives slope -0.948 for ULA
(R² = 0.998) and -1.032 for MALA (R² = 0.999), bracketing the Kramers prediction of -1.
